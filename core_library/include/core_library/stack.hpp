...
class Stack {
  ...
  public:
    ...
    void push(const T& value) {
      if (isFull()) throw overflow_error("Stack overflow: 1MB limit reached.");
      data[++top] = value;
    }

    T pop() {
      if (isEmpty()) throw underflow_error("Stack underflow: no elements to pop.");
      return data[top--];
    }
    ...
    bool isFull() const {
      return top >= static_cast<int>(MAX_ELEMENTS) - 1;
    }
    ...
    void loadDataIntoStack() {
      ...
      for (string line : readBuffer) {
        ...
        SupplyItem item = SupplyItem(tempArr[0], tempArr[1], std::stoi(tempArr[2]), tempArr[3]);
